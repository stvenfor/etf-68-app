...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional etf68-weekly-macd-ma-backtest-*.json to gate 技术候选",
    )
...
def main() -> int:
    ...
    weekly_by_code: dict[str, Any] | None = None
    if args.weekly_backtest is not None:
        weekly_payload = json.loads(args.weekly_backtest.read_text(encoding="utf-8"))
        weekly_by_code = {str(row["code"]): row for row in weekly_payload.get("rows", [])}
    report = build_report(
        seed=seed,
        ...
        weekly_backtest_by_code=weekly_by_code,
    )
